...
namespace Group4.AbpZeroTemplate.Web.Core.Cars
{
    ...
    {
        IDictionary<string, object> CAR_Upd(SearchCarDto input);
        IDictionary<string, object> CAR_App(int id, string checkerId);
        SearchCarDto CAR_ById(int id);
        IDictionary<string, object> CAR_Ins(SearchCarDto input);
        IDictionary<string, object> CAR_Del(int id);
        List<SearchCarDto> CAR_Search(SearchCarDto input);
    ...
    public class CarAppService : BaseService, ICarAppService
    {
        ...
        public SearchCarDto CAR_ById(int id)
        {
            return procedureHelper.GetData<SearchCarDto>("CAR_ById", new
            {
                Id = id
            }).FirstOrDefault();
        }
        ...
        public IDictionary<string, object> CAR_Ins(SearchCarDto input)
        {
            return procedureHelper.GetData<dynamic>("CAR_Ins", input).FirstOrDefault();
        }

        public List<SearchCarDto> CAR_Search(SearchCarDto input)
        {
            return procedureHelper.GetData<SearchCarDto>("CAR_Search", input);
        }

        public IDictionary<string, object> CAR_Upd(SearchCarDto input)
        {
